...
const AudioVisualizer: React.FC<VisualizerProps> = ({ canvasWidth, canvasHeight, websocketUrl }) => {
  ...
  const drawVisualizer = () => {
    ...
    [cite_start]// 1. Smooth Animation at 60 FPS [cite: 8]
    animationRef.current = requestAnimationFrame(drawVisualizer);
    
    [cite_start]// 2. Analyze frequency data [cite: 6]
    ...
    [cite_start]// 3. Render a circular frequency visualizer [cite: 7]
    for (let i = 0; i < numBars; i++) {
        const dataIndex = Math.floor(i * stepSize);
        
        [cite_start]// Magnitude (0-255) reacts instantly to volume/frequency changes [cite: 9]
  ...
  const initAudioAndStream = async () => {
    ...
    try {
      [cite_start]// 1. Access microphone using MediaStream API [cite: 5]
      ...
      processor.onaudioprocess = (e) => {
        ...
        [cite_start]// 3. Send audio chunk to backend immediately [cite: 15]
  ...
  return (
    <div className="audio-visualizer-container">
      [cite_start]{/* Visualizer Canvas [cite: 7] */}
      <canvas 
        ref={canvasRef} 
        width={canvasWidth} 
        height={canvasHeight} 
        [cite_start]// UI should be clean and visually appealing [cite: 10]
        style={{ borderRadius: '50%', background: '#121629', boxShadow: '0 0 15px #bb64e6' }}
      />
    ...
    <React.Fragment> // <-- Wrap adjacent elements in a Fragment
      ...
      <div id="transcriptionOutput">
            ...
            <p id="partialTranscription">{transcription}</p>
      </div>
    </div>
  );
};
